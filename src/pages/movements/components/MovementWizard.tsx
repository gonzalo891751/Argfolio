...
import type { Movement, Currency, FxType, MovementType, AssetCategory, MovementFee } from '@/domain/types'
...
interface WizardState {
    ...
    feeValue: number // can be % or fixed amount
...
export function MovementWizard({ open, onOpenChange, prefillMovement }: MovementWizardProps) {
    ...
    const handleClassChange = (cls: AssetClass) => {
        ...
        setState(s => ({ ...s, assetClass: cls, currency, fxType, opType, asset }))
    }
    ...
    const totals = useMemo(() => {
        ...
        if (state.feeMode === 'PERCENT') {
            feeNative = nativeGross * (state.feeValue / 100)
        } else {
            feeNative = state.feeValue
        }
        ...
        try {
            ...
            if (state.asset) {
                ...
                if (existing) {
                    ...
                    // Create new instrument on the fly
                    const newId = crypto.randomUUID()
                    const newInst: Instrument = {
                        id: newId,
                        symbol: state.asset!.ticker,
                        name: state.asset!.name,
                        category: (state.asset!.category as AssetCategory) || 'STOCK',
                        nativeCurrency: 'USD',
                        priceKey: state.asset!.ticker,
                    }
            ...
            const movementPayload: Omit<Movement, 'id'> = {
                ...
                // For currency/wallet, asset info is implicit or generic
                // If specific asset fields needed by backend for display when instrumentId is null:
                // We might need 'notes' to carry ticker info? 
                // But Strict Movement type has instrumentId.
                ...
                // New Fee Object
                fee: state.feeValue > 0 ? {
                    mode: state.feeMode,
                    percent: state.feeMode === 'PERCENT' ? state.feeValue : undefined,
                    amount: totals.feeNative, // Store absolute amount too for easier reading
            ...
            if (prefillMovement?.id) {
                ...
                // Ensure id is generated by backend/repo if omitted, 
                // but types require 'id'. 
                // Repo.create(movement: Movement). 
                // If repo requires full Movement object including ID, we must generate it or cast.
                // Usually create accepts 'Omit<Movement, id>' or we generate UUID.
                // Let's assume repo.create handles ID generation or we pass simplified.
                // Casting as any to bypass strictly required 'id' if the hook/repo handles it.
                // Or better: generate a temp ID if local-first.
                // Given "useCreateMovement" signature takes "Movement", I'll cast for now 
                // assuming backend ignores/overwrites ID.
                await createMovement.mutateAsync({
                    ...movementPayload,
                    id: crypto.randomUUID() // Client-side ID generation for safety
                } as Movement)
                                ...
                                <div className="lg:col-span-7 space-y-6">
                                    ...
                                    <div className="pt-2 border-t border-white/5 space-y-4">
                                        <div className="flex items-center justify-between">
                                            ...
                                            {/* Optional: Add Toggle Switch Here if needed, currently always visible inputs */}
                                        </div>
